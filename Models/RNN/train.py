...
######## __LSTM__ ########
parser.add_argument('--embedding', action='store', default=8, type=int,
                    help='embedding size')
parser.add_argument('--embed', action='store', default=1, type=int,
                    help='embedding toggle')
parser.add_argument('--hidden', action='store', default=4, type=int,
                    help='lstm hidden dim')
parser.add_argument('--layers', action='store', default=1, type=int,
                    help='num lstm layers')
parser.add_argument('--attention', action='store', default=0, type=int,
                    help='attention head count')
parser.add_argument('--drop', action='store', default=0, type=float,
                    help='droprate')
...
parser.add_argument('--lr', action='store', default=0.001, type=float,
                    help='learning rate')
parser.add_argument('--sgd', action='store', default=False, type=bool,
                    help='replace Adam with SGD')
parser.add_argument('--b1', action='store', default=0.9, type=float,
                    help='momentum')
parser.add_argument('--b2', action='store', default=0.999, type=float,
                    help='momentum')
parser.add_argument('--l2', action='store', default=0.001, type=float,
...
if __name__ == '__main__':
    ...
    if args.embed:
        arch_name = '_'.join(['emb'+str(args.embedding), 'h'+str(args.hidden), 'l'+str(args.layers), 'a'+str(args.attention)])
    else:
        arch_name = '_'.join(['h'+str(args.hidden), 'l'+str(args.layers), 'a'+str(args.attention)])
    ...
    '''LSTM'''
    model = RNN(embedding=args.embedding, input_size=190, output_size=10, hidden_size=args.hidden, num_layers=args.layers, 
             batch_size=args.batch, attention=args.attention, embed=bool(args.embed), droprate=args.drop).to(args.device)

    criterion = nn.BCELoss(reduction='none')

    if args.sgd:
        optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.b1, weight_decay=args.l2)
    else:
