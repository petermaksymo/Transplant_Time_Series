...
class Dataset(data.Dataset):
    ...
    def __init__(self, IDs, loaded_data, inc_year=False):
...
def make_train_loader(train_path, train_data, batch_size, shuffle, collate_fn, small=False, cv=1):
    class_totals = []
    for class_name in CLASSES:
        class_totals.append(len(fnmatch.filter(os.listdir(train_path+class_name), '*.pt')))

    min_val = 0
    train_indices = []
    for amt in class_totals:
...
def get_train_weights(train_path, train_data, year, precomputed=True):
    ...
    for epoch in range(3):
        train_loader = make_train_loader(train_path, train_data, batch_size=1024, shuffle=True, collate_fn=collate_fn)
...
def get_valid_weights(valid_indices, valid_data, year, precomputed=True):
    ...
    val_dataset = Dataset(valid_indices, valid_data)
    val_loader = DataLoader(val_dataset, batch_size=1024, shuffle=True, collate_fn=collate_fn,
                            generator=torch.Generator(device='cuda'))
